...
func main() {
	...
	r, err := root.OpenRoot(filepath.Dir(ex))
	if err != nil {
		log.Fatal(err)
	}
	defer r.Close()

	p, err := io.LoadPlaytimes(r.FS())
	if err != nil {
		log.Fatal(err)
	}

	complete := false
	out, err := r.CreateTemp("System/Played Games", "playtimes_*.tmp")
	if err != nil {
		log.Fatal(err)
	}
	defer func() {
		...
		if complete { // Overwrite the original with the temp file if successful; delete it if not.
			err = r.Rename(filepath.Join("System/Played Games/", filepath.Base(out.Name())), "System/Played Games/playtimes.bin")
		} else {
